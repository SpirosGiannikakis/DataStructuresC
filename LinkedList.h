...
public:
	...
	~list() // Destructor
	{
		...
		while (head != nullptr)
		{
			...
			delete head;
			head = tempo;
			std::cout << "Destructor Called " << "Destroying node number " << index << '\n';
			index++;
		}
	}
	...
	void display() //Δηλωση της μεθοδου display η οποια θα εκτυπωνει το πεδιο data όλων των κομβων της λιστας
	{
		...
		{
			std::cout << temp->data << " "; // εκτυπωση του πεδιου data των κομβων
	...
	int length() const {
		...
		std::cout << "The lists length is: " << len << '\n';
		return len;
	}
	...
	T find(int k)
	{
		...
		if (current) {
			std::cout << "In index point: " << k << " || " << " The value is: " << current->data << '\n';
			return current->data;
		}
	}
	...
	int search(const T& x) const {
		...
		if (current) {
			std::cout << "The value: " << x << " || " << "Was found in index point: " << index << '\n';
			return index;
		}
		return 0;
	}
	...
	T const findmax() {
		...
		std::cout << "The MAXIMUM value found in the list is: " << max << '\n';
		return max;
	}
	...
	T const findmin() {
		...
		std::cout << "The MINIMUM value found in the list is: " << min << '\n';
		return min;
	}
